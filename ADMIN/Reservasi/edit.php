...
$reservasi = query("SELECT * FROM reservasi LEFT JOIN layanan USING(id_layanan) WHERE id_reservasi = $id")[0];
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            <input type="text" class="form-control" name="nama" id="nama" placeholder="Nama"
                                value="<?php echo $reservasi['nama'] ?>" require>
                        </div>

                        <div class="mb-3">
                            <input type="text" class="form-control" name="nohp" id="nohp" placeholder="Nomer Telepon"
                                value="<?php echo $reservasi['no_hp'] ?>" require>
                        </div>

                        <div class="mb-3">
                            <select name="layanan" class="form-select" id="layanan" aria-label="Default select example">
                                <option value="">Jenis Perawatan</option>
                                <?php while($row = mysqli_fetch_assoc($layanan)){?>
                                <option value="<?php echo $row['id_layanan'] ?>"
                                    <?php echo ($row['id_layanan'] == $reservasi["id_layanan"])? 'selected': ''; ?>>
                        ...
                        <div class="mb-3">
                            <input type="date" class="form-control" name="tanggal" id="tanggal" placeholder="Tanggal"
                                value="<?php echo $reservasi['tanggal'] ?>" require>
                        </div>

                        <div class="mb-3">
                            <input type="text" class="form-control" name="jam" id="jam" placeholder="jam / Waktu"
                                value="<?php echo $reservasi['jam'] ?>" require>
                        </div>

                        <div class="mb-3">
                            <button class="btn btn-outline-primary text-center mt-2" type="submit" name="edit"
                                id="edit">Edit</button>
                        </div>
                    </form>
                </div>
